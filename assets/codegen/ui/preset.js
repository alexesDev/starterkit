...
// Ported from github.com/trip2g/mol_graphql (codegen/preset.js); symbols are
// derived relative to `molRoot` (assets/ui), which is mounted into the mam
// workspace as the `starterkit` package.
//
// assets/ui/users/list.graphql -> assets/ui/users/list.graphql.ts ($starterkit_users_list)
//
// Each output is wrapped in `namespace $` - the mam builder compiles it as an
// ordinary .ts, no builder changes ("two builders through a file seam").
...
module.exports = {
	buildGeneratesSection: options => {
			...
			for (const def of doc.document.definitions) {
				...
				if (fragments[name]) {
					throw new Error(
						...
						`(a/b/c.graphql -> starterkit_a_b_c), which cannot collide.`,
					)
				}
				fragments[name] = {
		...
		// `namespace $`, referenced by all generated files
